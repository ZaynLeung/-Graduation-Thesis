...
输入：每一帧内所有人体关节的坐标 。
处理机制：首先将每个关节坐标线性投影为高维向量，并加入可学习的空间位置嵌入以保留关节类别信息 。作用：利用自注意力机制在单帧内捕捉不同关节之间的空间拓扑依赖关系 。
...
\begin{figure}[t]
  ...
  \captionsetup{font=footnotesize}
  \caption{ Temporal-Spatial Transformer structure. 
  The input is a sequence of 3D human hand poses, which are first processed through a spatial transformer to extract inter-joint features, 
  and then fed into a temporal transformer to capture the temporal dynamics across frames. Finally, the output is regressed into robotic joint angles.
  (a) Temporal-Spatial Transformer module; (b) Spatial Transformer module; (c) Transformer encoder.
  }
...
而是从全局视角理解一段完整的动作语义。例如，在处理一个完整的“抓取并举起”动作时，时间编码器能够确保指尖在闭合过程中的平滑过渡，
以及与手臂抬起动作的时序协同。由于引入了时间编码器，TransRetarget 克服了传统重定向方法中常见的“动作闪烁”问题。
